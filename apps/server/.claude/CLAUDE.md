...
Use helper functions from `src/common/error-messages.ts`:

- `notFound(entity, errorCode)` → `NotFoundException`
...
Response format: `{ errorCode: string, message: string }`. When adding a new ErrorCode, add it to the enum in `@clinio/shared`.
