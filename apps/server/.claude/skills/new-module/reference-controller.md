...
- `@Controller("resource")` — lowercase plural
- `@ApiTags("Name")` for Swagger grouping
- Every endpoint has `@ApiOperation({ operationId: "..." })` — required for codegen
- Validate UUID params: `@Param("id", ParseUUIDPipe)`
- Validate body: `@UsePipes(new ZodValidationPipe(schema))`
- Always map entities to DTOs via Mapper before returning
