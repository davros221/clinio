...
@Injectable()
export class ParseEnumArrayPipe<T extends Record<string, string>> implements PipeTransform<string | string[], T[keyof T][]> {
  ...
  transform(value: string | string[] | undefined): T[keyof T][] {
    if (value === undefined || value === "") {
      throw badRequest(`Expected one or more values from: ${this.validValues.join(", ")}`);
    }
    ...
    if (invalid.length > 0) {
      throw badRequest(`Invalid value(s): ${invalid.join(", ")}. Expected: ${this.validValues.join(", ")}`);
    }
